...
while hide=1:
    ...
    hub = PrimeHub()
    colorE = ColorSensor(Port.E) 
    colorF = ColorSensor(Port.F)
    smallMotorC = Motor(Port.C)
    smallMotorD = Motor(Port.D)
...
class DriveBase:
    ...
    def lineFollower(self, distance, startspeed, maxspeed, endspeed, sensorPort, side, addspeed = 0.2, brakeStart = 0.7 , stopMethod=None, generator = None, stop = True):
        ...
        while loop:
            ...
            steering = (((change * kP_L) + (integral * kI_L) + (kD_L * (change - old_change)))) * invert
    ...
    def gyroStraightDrive(self, distance, startspeed, maxspeed, endspeed, addspeed = 0.3, brakeStart = 0.7, stopMethod=None, offset = 0, generator = None, stop = True):
        ...
        while loop:
            ...
            currenSteering = (change * kP + integral * kI + kD * (change - old_change)) + offset + steeringSum*0.02
        ...
        def loop(self):
            ...
            if colorE.get_reflected_light() < self.lightValue:
                    ...
                    if(self.detectedLineDistance == 0):
                        ...
                        self.detectedPort = Port.E

                    elif self.detectedPort == Port.F:
                ...
                if(self.detectedLineDistance == 0):
                    ...
                    self.detectedPort = Port.F

                elif self.detectedPort == Port.E:
...
def motorResistance(speed, port, resistancevalue):
    ...
    if port == "C":
        ...
        print("wrong port selected. Select C or D")
        return
...
def pidCalculationLight(speed):
    ...
    kP_L = -0.04 * speed + 4.11
    kD_L = 0.98 * speed - 34.2
...
hub.button.right.callback(breakFunction)
gyroValue = 0
...
main = True
hub.motion.yaw_pitch_roll(0)
db = DriveBase(hub, Port.A, Port.B) #this lets us conveniently hand over our motors (A: left driver; CB: right driver). This is necessary for the cancel function
...
while main:
    ...
    if hub.right_button.is_pressed(): #press right button to cycle through programs. cycling back isn't supported yet, but we are working on reallocating the buttons in the file system
        wait(150) #waiting prevents a single button press to be registered as multiple clicks
